...
if [ ! -d "/config/letsencrypt" ]; then
    ...
    certbot certonly --non-interactive --dns-cloudflare --dns-cloudflare-credentials "/config/cloudflare.ini" --agree-tos --staging --config-dir /config/letsencrypt -d "$DOMAIN" --cert-name https --email "$EMAIL" 2>&1 | while read line; do log_for_task "CERTBOT" "$line"; done
